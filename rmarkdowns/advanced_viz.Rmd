...
scatter_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() + theme_minimal() + scale_fill_viridis_d() +
  labs(title = "SDG 3 & 4 Score Comparisons", x = "SDG 3 Score", y = "SDG 4 Score", fill = "Region")
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Greens", goal_3_score)(goal_3_score), label = mytext)
```
