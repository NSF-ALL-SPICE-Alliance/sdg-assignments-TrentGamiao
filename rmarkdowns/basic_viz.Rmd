...
```{r}
ggplot(data = sdr_data, aes(x = goal_11_score, y = goal_8_score, color = goal_1_score)) + geom_point()
```
Interpretation:
We can see that countries with higher SDG 1 (no poverty) scores tend to also have higher SDG 8 (work and economic growth) and SDG 11 (sustainable cities and communities) scores. While, countries with lower SDG 1 scored tend to have mid to low SDG 8 and SDG 11 scores.
...
ggplot(data = mena_sdr_data, aes(x = goal_1_score, y = country)) + geom_bar(stat = "identity")
```
...
```{r}
ggplot(sdr_data, aes(x = goal_11_score, fill = regions_used_for_the_sdr)) + geom_histogram(binwidth = 5, color="black")
```
Interpretation:
Most countries fall beween the 75-90 score range for SDG 11. We can see that countries in the OECD and LAC are on the higher end of the scores. While countries in the Sub-Saharan Africa fall on the lower end of scores.
