...
```{r}
rf_accounts <- randomForest(normalized_score_sdg8_accounts ~ .,
...
```{r}
importance_df <- as.data.frame(rf_accounts$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Adults with a Financial Account",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_matmort, pred.var = "normalized_score_sdg3_neonat", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_matmort, pred.var = c("normalized_score_sdg3_neonat", "normalized_score_sdg12_ewaste"))
# Default PDP
plotPartial(pd)
```
...
After increasing the number of clusters to 5, we can see that there is a clear split between clusters 4 & 5, and clusters 1, 2, & 3. We can see from country_clusters that most of the better performing countries fall in clusters 1 & 3, while worse performing countries fall in cluster 4 & 5.
...
I changed our indicator to normalized_score_sdg8_accounts. After running our random forest, we ended up with 75.87% Var explained. When looking at our top 10 importance indicators, we ended up with sdg3_neonat, sdg5_u5mort, and sdg12_ewaste as our top 3 indicators with the highest importance. Using sdg3_neonat and sdg12_ewaste as our prediction variables, we can see that sdg3_neonat impacted the prediction score a lot more than sdg12_ewaste did. It showed that when sdg3_neonat had a positive relationship with sdg8_accounts. sdg12_ewaste only seemed to affect sdg8_accounts when sdg12_ewaste was above 80.
